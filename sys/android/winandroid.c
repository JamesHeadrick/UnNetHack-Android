...
#define TUTOR_VALUE 0xbeef
		...
		if(flags.initrole < 0)
		{
			...
			any.a_int = TUTOR_VALUE;
			and_add_menu(win, NO_GLYPH, MENU_DEFCNT, &any, 0, 0, ATR_NONE, "Tutorial mode", MENU_UNSELECTED);
			any.a_int = randrole()+1;
			and_add_menu(win, NO_GLYPH, MENU_DEFCNT, &any, '*', 0, ATR_NONE, "Random", MENU_UNSELECTED);
			for(i = 0; roles[i].name.m; i++)
			{
				if(ok_role(i, flags.initrace, flags.initgend, flags.initalign))
				{
					...
					if(thisch == lastch)
						thisch = highc(thisch);
					and_add_menu(win, NO_GLYPH, MENU_DEFCNT, &any, thisch, 0, ATR_NONE, roles[i].name.m, MENU_UNSELECTED);
					lastch = thisch;
				}
			}
			...
			if(result > 0) {
				if(selected[0].item.a_int == TUTOR_VALUE)
					tutorial = TRUE;
				else
					tutorial = FALSE;
		...
		if(tutorial)
		{
			...
			start_menu(win);
			any.a_int = 1;
			and_add_menu(win, NO_GLYPH, MENU_DEFCNT, &any, 'v', 0, ATR_NONE,
				 "lawful female dwarf Valkyrie (uses melee and thrown weapons)",
				 MENU_UNSELECTED);
			any.a_int = 2;
			and_add_menu(win, NO_GLYPH, MENU_DEFCNT, &any, 'w', 0, ATR_NONE,
				 "chaotic male elf Wizard      (relies mostly on spells)",
				 MENU_UNSELECTED);
			any.a_int = 3;
			and_add_menu(win, NO_GLYPH, MENU_DEFCNT, &any, 'R', 0, ATR_NONE,
		...
		if(flags.initrace < 0)
		{
			...
			for(i = 0; races[i].noun; i++)
				...
				{
					any.a_int = i + 1; /* must be non-zero */
					and_add_menu(win, NO_GLYPH, MENU_DEFCNT, &any, races[i].noun[0], 0, ATR_NONE, races[i].noun, MENU_UNSELECTED);
				}
		...
		if(flags.initgend < 0)
		{
			...
			for(i = 0; i < ROLE_GENDERS; i++)
				...
				{
					any.a_int = i + 1;
					and_add_menu(win, NO_GLYPH, MENU_DEFCNT, &any, genders[i].adj[0], 0, ATR_NONE, genders[i].adj, MENU_UNSELECTED);
				}
		...
		if(flags.initalign < 0)
		{
			...
			for(i = 0; i < ROLE_ALIGNS; i++)
				...
				{
					any.a_int = i + 1;
					and_add_menu(win, NO_GLYPH, MENU_DEFCNT, &any, aligns[i].adj[0], 0, ATR_NONE, aligns[i].adj, MENU_UNSELECTED);
				}
...
int do_ext_cmd_menu()
{
	...
	{
		any.a_int = i+1;
		and_add_menu(wid, NO_GLYPH, MENU_DEFCNT, &any, accelerator, 0, ATR_NONE, ptr, FALSE);
