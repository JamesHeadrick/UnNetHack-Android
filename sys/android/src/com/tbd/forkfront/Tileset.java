...
public class Tileset
{
	...
	public void updateTileset(SharedPreferences prefs, Resources r)
	{
		...
		if(!TTY)
		{
			...
			int id = mContext.getResources().getIdentifier("overlays", "drawable", mPackageName);
			if(id > 0)
			{
				try {
			...
			if(mBitmap == null) {
				Toast.makeText(mContext, "Could not load tileset bitmap: " + tilesetName, Toast.LENGTH_LONG).show();
				TTY = true;
			} else if(mOverlay == null) {
				Toast.makeText(mContext, "Could not load overlays bitmap", Toast.LENGTH_LONG).show();
				TTY = true;
			} else {
				mnCols = mBitmap.getWidth() / mTileW;
				if(mnCols <= 0)
				{
					Toast.makeText(mContext, "Tileset bitmap too narrow (" + mBitmap.getWidth() + ") for tile width (" + mTileW + ")", Toast.LENGTH_LONG).show();
					TTY = true;
				}
			}
		}
	...
	private void loadFromResources(String tilesetName, Resources r)
	{
		...
		// Try to find as a resource drawable first
		int id = r.getIdentifier(tilesetName, "drawable", mPackageName);
		if(id > 0)
		{
			try {
				BitmapDrawable bmpDrawable = (BitmapDrawable) r.getDrawable(id);
		...
		// If that fails, try to find as an asset
		try
		{
			mBitmap = BitmapFactory.decodeStream(mContext.getAssets().open(tilesetName + ".png"));
		}
		catch(IOException e)
		{
			// Try stripping the dimension suffix (e.g. unabsurd64_64 -> unabsurd64)
			int i = tilesetName.lastIndexOf('_');
			if(i > 0)
			{
				try
				{
					mBitmap = BitmapFactory.decodeStream(mContext.getAssets().open(tilesetName.substring(0, i) + ".png"));
				}
