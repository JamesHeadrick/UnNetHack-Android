...
public class NH_State
{
	...
	// ____________________________________________________________________________________
	private void updateSystemUiVisibilityFlags(final SharedPreferences prefs)
	{
		...
		decorView.post(new Runnable() {
			@Override
			public void run() {
				if (mContext.isFinishing() || (Build.VERSION.SDK_INT >= Build.VERSION_CODES.JELLY_BEAN_MR1 && mContext.isDestroyed())) return;
				...
				if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.R) {
					...
					if (controller != null && decorView.isAttachedToWindow()) {
						try {
							if (isImmersive || isFullscreen) {
								int types = 0;
								if (isFullscreen) types |= WindowInsets.Type.statusBars();
								if (isImmersive) types |= WindowInsets.Type.navigationBars() | WindowInsets.Type.statusBars();
							...
							// Workaround for internal InteractionJankMonitor issues on some Android versions
							Log.print("Error setting system UI visibility: " + e.getMessage());
						}
					}
				} else {
					int fullscreenFlag = isFullscreen ? WindowManager.LayoutParams.FLAG_FULLSCREEN : 0;
					window.setFlags(fullscreenFlag, WindowManager.LayoutParams.FLAG_FULLSCREEN);
