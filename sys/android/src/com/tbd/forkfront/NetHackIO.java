...
public class NetHackIO
{
	...
	private String getDumplogDir() {
		...
		if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.R) {
			// API 30+: Write to public Documents directory (requires MANAGE_EXTERNAL_STORAGE)
			file = new File(Environment.getExternalStoragePublicDirectory(Environment.DIRECTORY_DOCUMENTS), "unnethack_logs");
		} else if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.KITKAT) {
			file = new File(mActivity.getExternalFilesDir(Environment.DIRECTORY_DOCUMENTS), "unnethack_logs");
		} else {
			file = new File(Environment.getExternalStorageDirectory(), "Documents" + File.separator + "unnethack");
		}

		try {
			if (!file.exists()) {
				boolean success = file.mkdirs();
				if (!success) Log.print("Failed to create log directory at: " + file.getAbsolutePath());
			}
		} catch (Exception e) {
			Log.print("Error creating dumplog dir" + e);
		}
