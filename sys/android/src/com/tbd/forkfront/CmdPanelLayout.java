...
public class CmdPanelLayout extends FrameLayout
{
	...
	private void loadPanels(SharedPreferences prefs)
	{
		...
		// Fresh install: no panel configured yet — initialize with defaults.
		// Also migrate old installs where pCmdString0 was set to the placeholder
		// string from config.xml ("default_cmd_panel") instead of real commands.
		if(!bResetPanel && !prefs.contains("pPortActive0"))
		{
			Log.print("first run panel init");
			resetPanels(editor);
		}
		else if(!bResetPanel)
		{
			String defaultPlaceholder = mContext.getResources().getString(R.string.defaultCmdPanel);
			String storedCmds = prefs.getString("pCmdString0", "");
			if(storedCmds.equals(defaultPlaceholder))
			{
				Log.print("migrating placeholder cmd panel");
				resetPanels(editor);
			}
		}
		...
		for(int iPanel = 0; iPanel < MAX_PANELS; iPanel++)
		{
			...
			if(bPortActive || bLandActive)
			{
				...
				// Ensure "menu" is always there if this is the only active panel
				if(iPanel == 0 && !panel.cmds.contains("menu"))
					panel.cmds = panel.cmds + " menu";
